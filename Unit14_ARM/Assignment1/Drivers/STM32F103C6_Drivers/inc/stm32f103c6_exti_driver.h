...
typedef struct{
	...
	EXTI_GPIO_Mapping EXTI_Map;				 		//specifies which interrupt line on which port and which pin
													//must be a value of @ref EXTI_GPIO_define

	uint8	EXTI_Trigger;							//specifies the triggering condition either rising, falling or both
													//must be a value of @ref EXTI_TRIGGER_define

	uint8	EXTI_EnableState;						//specifies wether to enable or disable interrupt
